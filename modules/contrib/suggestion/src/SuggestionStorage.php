...
class SuggestionStorage {
  ...
  public static function getCount($ngram = '') {
    if ($ngram) {
      return db_query("SELECT COUNT(*) FROM {suggestion} WHERE ngram LIKE :ngram", [':ngram' => $ngram])->fetchField();
    }
  ...
  public static function getKeywords() {
    return db_query("SELECT ngram FROM {suggestion} WHERE src & :src ORDER BY ngram ASC", [':src' => self::PRIORITY_BIT])->fetchCol();
  }
  ...
  public static function getNgramQty($ngram = '') {
    return db_query("SELECT IFNULL(SUM(qty), 0) FROM {suggestion} WHERE ngram = :ngram", [':ngram' => $ngram])->fetchField();
  }
